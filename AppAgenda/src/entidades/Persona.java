...
@Table(name = "PERSONA")
@XmlRootElement
// Genera consultas predefinidas ( muestra "objeto" desde una columna )
@NamedQueries({
...
public class Persona implements Serializable {
    ...
    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY) // Indica que es clave primaria ( id )
    @Basic(optional = false) // Indica que es de caracter obligatorio
    @Column(name = "ID") // anotaciones de JPA | relacion entre atributos y tablas
    private Integer id; // Atributo/propiedades
    @Basic(optional = false) // Indica que es de caracter obligatorio
    @Column(name = "NOMBRE")
    ...
    @Column(name = "FOTO")
    private String foto;
    @JoinColumn(name = "PROVINCIA", referencedColumnName = "ID") // Provincia es clave foranea a id de tabla provincia
    @ManyToOne(optional = false) // Indica que es de caracter obligatorio
    private Provincia provincia; // Atributo/propiedades --> En SQl es un integer, ahora es de tipo Provincia ( por ser clave foranea )

    // Constructores ( sin parametros | con 1 parametro id | con parametros id, nombre y apellidos )
    public Persona() {
    }
    ...
    @Override // Metodo hashCode ( compara objetos )
    public int hashCode() {
        int hash = 0;
